...
for(long i=0; i<1; i++){ 
        ...
        #pragma omp parallel num_threads(6)
        {
            ...
            calculate_incidence_unit_vec(&surface, sat_coor_transmitter);  

            calculate_incidence_angle(&surface, sat_coor_transmitter);  

            calculate_reflected_unit_vec(&surface, sat_coor_receiver); 

            calculate_ponderacio(&surface);  

            calculate_propagation(&surface);  
