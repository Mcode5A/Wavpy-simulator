...
//============================== VARIABLES
	...
	double c0, c1, cov00, cov01, cov11, chisq, dt_ext, dt, coef_left, coef_right, twopifreq, sigma_y, filter_val;
...
//============================= Add "tail" and connect both waveform's ends
	wav_tail_length = 2;
	while(wav_tail_length <= wav_length*tail_factor){
	...
	left_lags = noise_lags;
	right_lags = tail_lags;
	noiseFloor = 0.0;
...
//============================== Interpolation: zero padding in spectrum
	...
	for(i=0; i<=wav_tail_length/2; i++){
		if(i <= wav_tail_length/int(sampling_rate/bandwidth_LPF)){
			filter_val = 1.0;
		}else{
			filter_val = 0.0;
		}
		spectrum_ext[i] = waveform[i]*double(interpfactor)*filter_val;
	...
	for(i=1; i<wav_tail_length/2; i++){
		...
		spectrum_ext[wav_ext_length - i] = waveform[wav_tail_length - i]*double(interpfactor)*filter_val;
...
//============================================================
//for(i=0; i<wav_ext_length; i++){
  //printf("%f %f %f %f %f\n", tau_ext[i], waveform_ext[i], waveform_first_deriv[i], waveform_second_deriv[i], spectrum_ext[i]);
//}
...
//============================================================
	if(get_deriv_tracks){
		for(i=0; i<fit_samples; i++){
			tau_deriv[i] = tau_ext[imin + i];
			wav_track_deriv[i] = waveform_ext[imin + i];
			wav_first_deriv[i] = waveform_first_deriv[imin + i];
			wav_second_deriv[i] = waveform_second_deriv[imin + i];
		}
		free(waveform);
		free(waveform_ext);
		free(spectrum_ext);
...
//============================================================
	...
	powSampleDer = waveform_ext[iposder];
	if(waveform_second_deriv[iposder] != 0.0){ //Compute position and power by means of linear interpolation
		if(waveform_second_deriv[iposder] > 0.0){
			iposder = iposder + 1;
		}
		posSampleDer = tau_ext[iposder] - waveform_second_deriv[iposder]*(tau_ext[iposder + 1] - tau_ext[iposder])/(waveform_second_deriv[iposder + 1] - waveform_second_deriv[iposder]);
		powSampleDer = waveform_ext[iposder] + (posSampleDer - tau_ext[iposder])*(waveform_ext[iposder + 1] - waveform_ext[iposder])/(tau_ext[iposder + 1] - tau_ext[iposder]);
	}
	positionDer = -c0/c1;
