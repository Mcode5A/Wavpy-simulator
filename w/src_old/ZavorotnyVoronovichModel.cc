/**
  \file ZavorotnyVoronovichModel.cc
  \brief Implementation of the ZaVoModel_GNSSR class.
*/
...
ZaVoModel_GNSSR::ZaVoModel_GNSSR( void ){
	...
	curvature_approx_flag = false;
	interferometric_flag = false;
	covariance_wav_flag = false;
	covariance_ddm_flag = false;
	dump_isolines_data = false;
	recompute_Lambda_flag = true;
    sigma0_to_one_flag = false;
    stare_processing_flag = false;
    use_transmitter_eirp = false;
    txr_eirp = 0.0;
	weight_cohPower = 0.0;
	size_ddm_stored[0] = 0;
	size_ddm_stored[1] = 0;
...
void ZaVoModel_GNSSR::set_stare_processing_mode( const char* namefile ){
	stare_processing_flag = true;
	stareProc_data_namefile = namefile;
	return;
}

void ZaVoModel_GNSSR::unset_stare_processing_mode( void ){
	stare_processing_flag = false;
	return;
}

void ZaVoModel_GNSSR::set_transmitter_EIRP( double txr_eirp_in ){
    if(txr_eirp_in > 0.0){
        use_transmitter_eirp = true;
        txr_eirp = txr_eirp_in;
    }else{
        printf("ERROR! Input EIRP should be > 0");
    }
	return;
}

void ZaVoModel_GNSSR::unset_transmitter_EIRP( void ){
	use_transmitter_eirp = false;
	return;
}

void ZaVoModel_GNSSR::compute_waveform( void ){
	int i, j, k, n, doppler_lines, central_doppler, interferometric;
    ...
    int wav_model_length_min = wav_length + 2*gnss_signal.lambda_size + gnss_signal.lambda_size/2;
	int wav_model_length = 2;
	while(wav_model_length < wav_model_length_min){
		wav_model_length = wav_model_length*2;
	}
	...
	double delay1, theta_origin, rad1, rad2;
	//Local constants
	...
	FILE *fpo_data;
    //Variables for stare processing
	FILE *fpo_stareProc;
    double q[3], first_q[3], prev_q[3];
    double q_mod, first_lon, first_lat, prev_lon, prev_lat;
    double **pow_q;
    double *pow_lon, *pow_lat;
    int *status_Doppler_cell;
    bool *prev_inside_Doppler_cell, *inside_Doppler_cell;
    //
	...
	if(((!covariance_wav_flag) && (!covariance_ddm_flag) && (len_cov_stored > 0)) || ((covariance_wav_flag) && (!covariance_ddm_flag) && (len_cov_stored != wav_length) && (len_cov_stored > 0)) || ((covariance_ddm_flag) && (len_cov_stored != wav_length*(1 + ddm_half_dopplers*2)) && (len_cov_stored > 0))){
	...
	if((covariance_wav_flag)||(covariance_ddm_flag)){
	...
	if(stare_processing_flag){
        if((fpo_stareProc = fopen(stareProc_data_namefile.c_str(), "w")) == NULL){
			printf("ERROR! stare_processing_file can't be opened.\n");
			return;
		}
		pow_q = (double **) malloc(doppler_lines*sizeof(double *));
        for(i=0; i<doppler_lines; i++){
            pow_q[i] = (double *) malloc(3*sizeof(double));
        }
        pow_lon = (double *) malloc(doppler_lines*sizeof(double));
        pow_lat = (double *) malloc(doppler_lines*sizeof(double));
        inside_Doppler_cell = (bool*) calloc(doppler_lines, sizeof(bool));
        prev_inside_Doppler_cell = (bool*) calloc(doppler_lines, sizeof(bool));
        status_Doppler_cell = (int*) calloc(doppler_lines, sizeof(int));
    }
	...
	if(recompute_Lambda_flag){
		gnss_signal.set_instrumental_params(sampling_rate, receiver_Down.get_filter_BB_BW(), 1);
	}else{
		gnss_signal.set_instrumental_params(sampling_rate, receiver_Down.get_filter_BB_BW(), 0);
	}
	...
	if(surface.get_wind_grid_status() || dump_isolines_data || stare_processing_flag){ //There is a wind grid to interpolate or dump isolines/stare data
		XYZ2GEOID(posS_ECEF_km, PhiLambdaH_S, ECEF2ENU_S);
	}
	//Computation of transmitted power PtGt
	if(use_transmitter_eirp){
        power_trans = pow(10.0, txr_eirp/10.0);
    }else{
	...
	if(weight_cohPower > 0.0){
		...
		pow_coherent = weight_cohPower*power_trans*reflect_pow*exp(-1.0*pow(4.0*PI_NUM*surface.sigma_z*cosIncidence*gnss_signal.frequency/C_LIGHT, 2.0))*directivity*sinc_Doppler[central_doppler]*pow(C_LIGHT*coherent_integration/(4.0*PI_NUM*gnss_signal.frequency*(norm3vec(posT_local) + norm3vec(posR_local))), 2.0)/atmospheric_loss;
	}
	...
	for(i=gnss_signal.lambda_size; i<(wav_model_length - gnss_signal.lambda_size); i++){
        ...
        if((stare_processing_flag)&&((i-gnss_signal.lambda_size)<(wav_length - gnss_signal.lambda_size/2 - 1))){
            for(n=0; n<doppler_lines; n++){
                memset(pow_q[n], 0, sizeof(double)*3);
            }
            memset(pow_lon, 0, sizeof(double)*doppler_lines);
            memset(pow_lat, 0, sizeof(double)*doppler_lines);
        }
		...
		for(j=0; j<num_angles; j++){
			...
			//Compute sigma0
			if(surface.get_wind_grid_status() || dump_isolines_data || ((stare_processing_flag)&&((i-gnss_signal.lambda_size)<(wav_length - gnss_signal.lambda_size/2 - 1)))){
            ...
            if(sigma0_to_one_flag){
                sigma_0 = 1.0;
            }else{
                sigma_0 = Sigma0_func(surface, n_scattered, n_incident, polarization, geometry.azimuthT, stare_processing_flag);
            }
			...
			if(dump_isolines_data){
				...
				isolineRecord.sigma_0 = float(10.0*log10(sigma_0));
				isolineRecord.directivity = float(10.0*log10(directivity));
			...
			if(j>0){
                if((stare_processing_flag)&&((i-gnss_signal.lambda_size)<(wav_length - gnss_signal.lambda_size/2 - 1))){
                    for(k=0; k<3; k++){
                        q[k] = n_scattered[k] - n_incident[k];
                    }
                    q_mod = norm3vec(q);
                    for(k=0; k<3; k++){
                        q[k] = q[k]/q_mod;
				...
				for(n=0; n<doppler_lines; n++){
					...
					//Computation of weighted mean q, lat and lon for stare processing
                    if((stare_processing_flag)&&((i-gnss_signal.lambda_size)<(wav_length - gnss_signal.lambda_size/2 - 1))){
                        if(fabs(doppler_x - double(n - central_doppler)*delta_doppler) < delta_doppler/2){
                            inside_Doppler_cell[n] = true;
                        }else{
                            inside_Doppler_cell[n] = false;
                        }
                        Check_status_Doppler_cell(inside_Doppler_cell[n], prev_inside_Doppler_cell[n], status_Doppler_cell[n]);
                        if(j<(num_angles - 1)){
                            for(k=0; k<3; k++){
                                pow_q[n][k] = pow_q[n][k] + dtheta*0.5*(ft[n]*q[k] + prev_ft[n]*prev_q[k]);
                            }
                            pow_lon[n] = pow_lon[n] + dtheta*0.5*(ft[n]*x_lon + prev_ft[n]*prev_lon);
                            pow_lat[n] = pow_lat[n] + dtheta*0.5*(ft[n]*x_lat + prev_ft[n]*prev_lat);
                        }else{
                            for(k=0; k<3; k++){
                                if(power_tau[n] > 0.0){
                                    pow_q[n][k] = (pow_q[n][k] + dtheta*0.5*(ft[n]*q[k] + first_ft[n]*first_q[k]))/power_tau[n];
                                }else{
                                    pow_q[n][k] = 0.0;
                                }
                            }
                            q_mod = norm3vec(pow_q[n]);
                            if(power_tau[n] > 0.0){
                                pow_lon[n] = (pow_lon[n] + dtheta*0.5*(ft[n]*x_lon + first_ft[n]*first_lon))/power_tau[n];
                                pow_lat[n] = (pow_lat[n] + dtheta*0.5*(ft[n]*x_lat + first_ft[n]*first_lat))/power_tau[n];
                            }else{
                                pow_lon[n] = 666.0;
                                pow_lat[n] = 666.0;
                            }
                            if(status_Doppler_cell[n] < 0){
                                status_Doppler_cell[n] = 1;
                            }
                            fprintf(fpo_stareProc, "%f %f %f %f %f %f %f %d\n", tau, double(n - central_doppler)*delta_doppler, pow_q[n][0]/q_mod, pow_q[n][1]/q_mod, pow_q[n][2]/q_mod, pow_lon[n], pow_lat[n], status_Doppler_cell[n]%2);
                        }
                        prev_inside_Doppler_cell[n] = inside_Doppler_cell[n];
                    }
					prev_ft[n] = ft[n];
				}
                if((stare_processing_flag)&&((i-gnss_signal.lambda_size)<(wav_length - gnss_signal.lambda_size/2 - 1))){
				...
				for(n=0; n<doppler_lines; n++){
                    ...
                    if((stare_processing_flag)&&((i-gnss_signal.lambda_size)<(wav_length - gnss_signal.lambda_size/2 - 1))){
                        if(fabs(doppler_x - double(n - central_doppler)*delta_doppler) < delta_doppler/2){
                            prev_inside_Doppler_cell[n] = true;
                            status_Doppler_cell[n] = -1;
                        }else{
                            prev_inside_Doppler_cell[n] = false;
                            status_Doppler_cell[n] = 0;
                        }
                    }
				}
                if((stare_processing_flag)&&((i-gnss_signal.lambda_size)<(wav_length - gnss_signal.lambda_size/2 - 1))){
                    for(k=0; k<3; k++){
                        first_q[k] = n_scattered[k] - n_incident[k];
                    }
                    q_mod = norm3vec(first_q);
                    for(k=0; k<3; k++){
                        first_q[k] = first_q[k]/q_mod;
                        prev_q[k] = first_q[k];
                    }
                    first_lon = x_lon;
                    first_lat = x_lat;
		...
		if(i == gnss_signal.lambda_size){
			for(n=0; n<doppler_lines; n++){
				power[n][i] = power_tau[n]/2.0; //The first iso-annuli covers half range area
			}
		}else{
			for(n=0; n<doppler_lines; n++){
				power[n][i] = power_tau[n];
	...
	if(stare_processing_flag){
        ...
        free(pow_lon);
        free(pow_lat);
        free(inside_Doppler_cell);
        free(prev_inside_Doppler_cell);
        free(status_Doppler_cell);
    }
	...
	if((interferometric_flag)||(covariance_wav_flag)||(covariance_ddm_flag)){ //Compute direct signal power at receiver
	...
	if((covariance_wav_flag)||(covariance_ddm_flag)){
        if(interferometric_flag){
            interferometric = 1;
        }else{
            interferometric = 0;
        }
		Compute_Covariance_DDM(power_trans*C_LIGHT/(sampling_rate*atmospheric_loss), power_direct, sampling_rate, coherent_integration, receiver_Down.get_filter_BB_BW(), pow(10.0, (receiver_Up.get_noise_pow_dBW()/10.0)), pow(10.0, (receiver_Down.get_noise_pow_dBW()/10.0)), delta_doppler, power, doppler_lines, wav_model_length, lambda_func, gnss_signal.lambda_size, waveform, ddm, cov, chol, wav_length, interferometric, cov_ddm_factor, covariance_ddm_flag);
		...
		if(interferometric_flag){ //Interferometric case
...
{
    if(current_cell == prev_cell){ //No change of status
        return;
    }else{
        if(current_cell){ //Entering into a Doppler cell 
            if(status == 0){
                status = 1; //Covering first Doppler cell (not-origin)
                return;
            }
            if(status == 3){
                status = 2; //Covering second Doppler cell (not-origin) -> Ambiguity
                return;
            }
            if(status == -2){
                status = -3; //Covering second Doppler cell (after origin) -> Not yet ambiguous
                return;
            }
        }else{ //Leaving a Doppler cell
            if(status == -1){
                status = -2; //Leaving Doppler cell at origin
                return;
            }
            if(status == -3){
                status = 2; //Leaving second Doppler cell (after origin) -> Ambiguity
                return;
            }
            if(status == 1){
                status = 3; //Leaving first Doppler cell (not-origin)
                return;
            }
        }
    }
