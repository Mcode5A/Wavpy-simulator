...
epsilon2_imag, rvv_real, rvv_imag, rhh_real, rhh_imag)
  ...
  epsilon1 = cmplx(epsilon1_real,epsilon1_imag, kind =4)
  epsilon2 = cmplx(epsilon2_real,epsilon2_imag, kind = 4)
...
! n= length of spectra (i.e.length of pos or/and negative)
...
!---------------------------------------------
  ...
  integer, INTENT(IN) :: n ! n is the length of spectra (pos/neg)
  real*8, dimension(n,n), INTENT(INOUT) :: psi
  real*8, dimension(n), INTENT(INOUT) :: kx, ky
  ...
  !------------------ k=0
  psi(:,:) = 0.0d0
  psi((n/2 + 1),(n/2 + 1)) = 0.0d0
  !------------------LOOP IN kx
  do i=1,n
     ix = i
     kx(ix) = (i - n/2 - 1)*deltak
     !---------------LOOP IN ky
     do ii=1,n
        iy = ii
        ky(iy) = (ii - n/2 - 1)*deltak
        if ((ix.eq.(n/2 + 1)).and.(iy.eq.(n/2 + 1))) goto 5
