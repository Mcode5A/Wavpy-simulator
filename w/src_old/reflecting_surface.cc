/**
    \file reflecting_surface.cc
    \brief Contains the implementation of the Reflecting_surface class.
*/
...
// Reference to external Fortran functions in Surface_routines.f90
extern "C" {
...
void Reflecting_surface::compute_sea_spectrum( int num_samples, double delta_k_in, double theta_deg, double omega ){
	...
	n = num_samples;
	if((nx_spec != num_samples)||(ny_spec != num_samples)){
		if(nx_spec > 0){
			for(i=0; i<nx_spec; i++){
		...
		nx_spec = num_samples;
		ny_spec = num_samples;
		surface_spectrum = (double **) malloc(nx_spec*sizeof(double *));
		for(i=0;i<nx_spec;i++){
			surface_spectrum[i] = (double *) malloc(ny_spec*sizeof(double));
		}
		kx_spec = (double*) malloc (nx_spec*sizeof(double));
		ky_spec = (double*) malloc (ny_spec*sizeof(double));
	}
	delta_k = delta_k_in;
	thetaD = theta_deg;
	ome = omega;
	psi = (double*) malloc (nx_spec*ny_spec*sizeof(double));
	sea_spectrum_elfouhaily_((double*)psi, (double*)kx_spec, (double*)ky_spec, &delta_k, &n, &wind_U10_speed, &thetaD, &ome);
	for(i=0; i<nx_spec; i++){
		for(j=0; j<ny_spec; j++){
			surface_spectrum[i][j] = psi[j*ny_spec + i];
		}
	}
	free(psi);
	return;
...
void Reflecting_surface::set_surf_spectrum( double* spectrum_in, int dim_x, int dim_y, double* kx_spec_in, int dim_kx, double* ky_spec_in, int dim_ky ){
	int i, j;
	if((dim_x <= 0)||(dim_y <= 0)){
		printf("ERROR! Dimension not valid (x or y <= 0)\n");
		return;
	}
	if(!((dim_x == dim_kx)&&(dim_y == dim_ky))){
		printf("ERROR! Inputs must have the same dim (spectrum[dim_x, dim_y], kx[dim_x] and ky[dim_y])\n");
		return;
	}
	if((nx_spec != dim_kx)||(ny_spec != dim_ky)){
		if(nx_spec > 0){
			for(i=0; i<nx_spec; i++){
				free(surface_spectrum[i]);
			}
	...
	for(i=0; i<nx_spec; i++){
		for(j=0; j<ny_spec; j++){
			surface_spectrum[i][j] = spectrum_in[i*nx_spec + j];
			if(i == 0){
				ky_spec[j] = ky_spec_in[j];
			}
		}
...
void Reflecting_surface::set_surf_spectrum_omnidir( double* spectrum_in_omnidir, int dim_x_omnidir, double* k_spec_in_omnidir, int dim_k_omnidir ){
	int i;
	if(dim_x_omnidir <= 0){
	...
	if((nx_spec != dim_x_omnidir)||(ny_spec != 0)){
		if(nx_spec > 0){
			for(i=0; i<nx_spec; i++){
				free(surface_spectrum[i]);
			}
...
void Reflecting_surface::get_surf_spectrum( int x, int y, double kx_ky_spectrum[3] ){
	...
	if((x>=nx_spec)||(y>=ny_spec)||(x<0)||(y<0)){
		printf("ERROR! Index out of spectrum limits (max_x=%d, max_y=%d)\n", nx_spec, ny_spec);
		return;
	}
...
void Reflecting_surface::compute_mss_from_spectrum( void ){
	int i, j;
	double accum_mss, accum_rmsz2, surface_spectrum_k_treshold;
	if(nx_spec == 0){
		printf("ERROR! Spectrum not memory\n");
		return;
	}
	accum_rmsz2 = 0.0;
	if(ny_spec == 0){
		accum_mss = 0.0;
		for(i=1; i<nx_spec; i++){
			if(kx_spec[i] <= kx_spec[i - 1]){
				printf("ERROR! A non-repetitive ascending sequence is mandatory for k-dim. Current MSS and sigma_z values are not valid.\n");
				return;
			}
			if((fabs(kx_spec[i]) <= k_threshold)&&(fabs(kx_spec[i - 1]) <= k_threshold)){
				accum_rmsz2 = accum_rmsz2 + 0.5*(surface_spectrum[i][0] + surface_spectrum[i - 1][0])*(kx_spec[i] - kx_spec[i - 1]);
				accum_mss = accum_mss + 0.5*(surface_spectrum[i][0]*kx_spec[i]*kx_spec[i] + surface_spectrum[i - 1][0]*kx_spec[i - 1]*kx_spec[i - 1])*(kx_spec[i] - kx_spec[i - 1]);
			}else{ //Apply linear interpolation at k_threshold if required for two cases
				if((fabs(kx_spec[i - 1]) < k_threshold)&&(fabs(kx_spec[i]) >= k_threshold)){ //Positive right-side case
					surface_spectrum_k_treshold = surface_spectrum[i - 1][0] + (k_threshold - kx_spec[i - 1])*(surface_spectrum[i][0] - surface_spectrum[i - 1][0])/(kx_spec[i] - kx_spec[i - 1]);
					accum_rmsz2 = accum_rmsz2 + 0.5*(surface_spectrum_k_treshold + surface_spectrum[i - 1][0])*(k_threshold - kx_spec[i - 1]);
					accum_mss = accum_mss + 0.5*(surface_spectrum_k_treshold*k_threshold*k_threshold + surface_spectrum[i - 1][0]*kx_spec[i - 1]*kx_spec[i - 1])*(k_threshold - kx_spec[i - 1]);
				}
				if((fabs(kx_spec[i]) < k_threshold)&&(fabs(kx_spec[i - 1]) >= k_threshold)){ //Negative left-side case
					surface_spectrum_k_treshold = surface_spectrum[i - 1][0] + (-k_threshold - kx_spec[i - 1])*(surface_spectrum[i][0] - surface_spectrum[i - 1][0])/(kx_spec[i] - kx_spec[i - 1]);
					accum_rmsz2 = accum_rmsz2 + 0.5*(surface_spectrum_k_treshold + surface_spectrum[i][0])*(k_threshold - fabs(kx_spec[i]));
					accum_mss = accum_mss + 0.5*(surface_spectrum_k_treshold*k_threshold*k_threshold + surface_spectrum[i][0]*kx_spec[i]*kx_spec[i])*(k_threshold - fabs(kx_spec[i]));
				}
			}
		}
		mss_x = accum_mss/2.0;
		...
		for(i=1; i<nx_spec; i++){
			if(kx_spec[i] <= kx_spec[i - 1]){
				printf("ERROR! A non-repetitive ascending sequence is mandatory for kx-dim. Current MSS and sigma_z values are not valid.\n");
				return;
			}
			for(j=1; j<ny_spec; j++){
				if(ky_spec[j] <= ky_spec[j - 1]){
					printf("ERROR! A non-repetitive ascending sequence is mandatory for ky-dim. Current MSS and sigma_z values are not valid.\n");
					return;
				}
				if(sqrt(kx_spec[i]*kx_spec[i] + ky_spec[j]*ky_spec[j]) < k_threshold){
					accum_rmsz2 = accum_rmsz2 + 0.25*(surface_spectrum[i][j] + surface_spectrum[i - 1][j] + surface_spectrum[i][j - 1] + surface_spectrum[i - 1][j - 1])*(kx_spec[i] - kx_spec[i - 1])*(ky_spec[j] - ky_spec[j - 1]);
					mss_x = mss_x + 0.25*(surface_spectrum[i][j]*kx_spec[i]*kx_spec[i] + surface_spectrum[i - 1][j]*kx_spec[i - 1]*kx_spec[i - 1] + surface_spectrum[i][j - 1]*kx_spec[i]*kx_spec[i] + surface_spectrum[i - 1][j - 1]*kx_spec[i - 1]*kx_spec[i - 1])*(kx_spec[i] - kx_spec[i - 1])*(ky_spec[j] - ky_spec[j - 1]);
					mss_y = mss_y + 0.25*(surface_spectrum[i][j]*ky_spec[j]*ky_spec[j] + surface_spectrum[i - 1][j]*ky_spec[j]*ky_spec[j] + surface_spectrum[i][j - 1]*ky_spec[j - 1]*ky_spec[j - 1] + surface_spectrum[i - 1][j - 1]*ky_spec[j - 1]*ky_spec[j - 1])*(kx_spec[i] - kx_spec[i - 1])*(ky_spec[j] - ky_spec[j - 1]);
				}
			}
		}
	}
