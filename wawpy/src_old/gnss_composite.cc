/**
    \file gnss_composite.cc
    \brief Implementation of the GNSS_composite class
*/
...
void GNSS_composite::compute_lambda_func( void ){
	if(lambda_size != 2*int(round(sampling_rate/GPS_CA_CHIP_RATE)) + 1){
		free(lambda_func);
		lambda_func = (double*) malloc (lambda_size*sizeof(double));
		lambda_size = 2*int(round(sampling_rate/GPS_CA_CHIP_RATE)) + 1;
	}
...
void GNSS_composite::set_lambda_func( double* lambda_in, int lambda_length ){
	int i;
	if((lambda_length <= 3)||(lambda_length == 2*(lambda_length/2))){
		printf("ERROR! lambda_length must be an odd number bigger than 3");
		return;
	}
	if(lambda_size != lambda_length){
		free(lambda_func);
		lambda_size = lambda_length;
		lambda_func = (double*) malloc (lambda_size*sizeof(double));
	}
	for(i=0; i<lambda_size; i++){
		lambda_func[i] = lambda_in[i];
	}
	lambda_updated = true;
	return;
}
