...
{
	double g, q, EclipticLongitude, Obliquity, TimeAfter2000Century, GST_h, GST0_h, UT_h, JulianDateAfter2000, JulianDate, sinDec, cosRAcosDec, sinRAcosDec, Dec, RA, EastLongitude;
...
void convolution_gsl( double vec1[], int size_vec1, double vec2[], int size_vec2, double vec_out[] )
{
	...
	//real = gsl_fft_real_wavetable_alloc(size_vec1);
	//hc = gsl_fft_halfcomplex_wavetable_alloc(size_vec1);
	//work = gsl_fft_real_workspace_alloc(size_vec1);
    //gsl_fft_real_transform(aux1, 1, size_vec1, real, work);
    //gsl_fft_real_transform(aux2, 1, size_vec1, real, work);
	...
	//gsl_fft_halfcomplex_inverse(vec_out, 1, size_vec1, hc, work);
...
void correlation_gsl( double vec1[], double vec2[], double vec_out[], int size_vec )
{
	...
	//gsl_fft_halfcomplex_wavetable * hc;
	//gsl_fft_real_workspace * work;
	//real = gsl_fft_real_wavetable_alloc(size_vec);
	//hc = gsl_fft_halfcomplex_wavetable_alloc(size_vec);
	//work = gsl_fft_real_workspace_alloc(size_vec);
    //gsl_fft_real_transform(aux1, 1, size_vec, real, work);
    //gsl_fft_real_transform(aux2, 1, size_vec, real, work);
    //gsl_fft_real_wavetable_free(real);
	vec_out[0] = aux1[0]*aux2[0];
	...
	//gsl_fft_halfcomplex_inverse(vec_out, 1, size_vec, hc, work);
	//gsl_fft_halfcomplex_wavetable_free(hc);
	//gsl_fft_real_workspace_free(work);
	free(aux1);
	free(aux2);
...
double Compute_Doppler_bandwidth( int samples, double* doppler_freqs, double* doppler_map, double pos_pow_Max[2] )
{
    ...
    // Maybe, the workspace pointer to an gsl_interp object should 
    // be initialized with gsl_interp_init(workspace, x[], y[], samples/2);
	gsl_interp_accel *accel;
    accel = gsl_interp_accel_alloc();
...
{
	int i, wav_tail_length, left_lags, right_lags;
	double real_part, imag_part, cos_del, sin_del, noiseFloor, coef_left, coef_right, c0, c1, cov00, cov01, cov11, chisq;
	double *wav_tail, *x_lfit, *y_lfit;
	//GSL FFT variables
	gsl_fft_real_wavetable * real;
	gsl_fft_halfcomplex_wavetable * hc;
	gsl_fft_real_workspace * work;
	//============================= Add "tail" and connect both waveform's ends
	wav_tail_length = 2*wav_length;
	wav_tail = (double*) malloc (wav_tail_length*sizeof(double));
	for(i=0; i<wav_length; i++){
		wav_tail[i] = waveform[i];
	}
	left_lags = 3;
	right_lags = wav_length/20;
	noiseFloor = 0.0;
	for(i=0; i<left_lags; i++){
		noiseFloor = noiseFloor + wav_tail[i];
	}
	noiseFloor = noiseFloor/double(left_lags);
	for(i=0; i<left_lags; i++){
		coef_left = double(left_lags - i)/double(left_lags);
		coef_right = double(i)/double(left_lags);
		wav_tail[i] = noiseFloor*coef_left + wav_tail[left_lags]*coef_right;
	}
	...
	for(i=0; i<right_lags; i++){
		...
		y_lfit[i] = wav_tail[int(x_lfit[i])];
	}
	...
	if((c1<0.0)&&((c0 + double(wav_tail_length - 1)*c1)<=noiseFloor)){
		for(i=wav_length; i<wav_tail_length; i++){
			wav_tail[i] = c0 + double(i)*c1;
			if(wav_tail[i] < noiseFloor){
				wav_tail[i] = noiseFloor;
			}
		}
	}else{
		for(i=wav_length; i<wav_tail_length; i++){
			...
			wav_tail[i] = wav_tail[wav_length - 1]*coef_left + noiseFloor*coef_right;
		}
	}
	free(x_lfit);
	free(y_lfit);
	//===========================================================
	real = gsl_fft_real_wavetable_alloc(wav_tail_length);
	hc = gsl_fft_halfcomplex_wavetable_alloc(wav_tail_length);
	work = gsl_fft_real_workspace_alloc(wav_tail_length);
	//FFT transform of sequence of real numbers (power waveform)
	gsl_fft_real_transform(wav_tail, 1, wav_tail_length, real, work);
	gsl_fft_real_wavetable_free(real);
	//Delay shift by means of *e^(-i2PIf*delay) in the frequency domain
	for(i=1; i<=((wav_tail_length - 1)/2); i++){
		cos_del = cos(2.0*PI_NUM*delay*double(i)/double(wav_tail_length));
		sin_del = sin(2.0*PI_NUM*delay*double(i)/double(wav_tail_length));
		real_part = wav_tail[2*i - 1]*cos_del + wav_tail[2*i]*sin_del;
		imag_part = wav_tail[2*i]*cos_del - wav_tail[2*i - 1]*sin_del;
		wav_tail[2*i - 1] = real_part;
		wav_tail[2*i] = imag_part;
	}
	//Inverse FFT
	gsl_fft_halfcomplex_inverse(wav_tail, 1, wav_tail_length, hc, work);
	gsl_fft_halfcomplex_wavetable_free(hc);
	gsl_fft_real_workspace_free(work);
	//Return waveform with original size
	for(i=0; i<wav_length; i++){
		waveform[i] = wav_tail[i];
	}
	free(wav_tail);
	return;
}
